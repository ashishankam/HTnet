...
def whole_face_block_coordinates():
    df = pandas.read_csv(r'E:\HTnet\HTNet-master\combined_3_class2_for_optical_flow.csv')
    m, n = df.shape
    base_data_src = r'E:\HTnet\HTNet-master\datasets\combined_datasets_whole'
    total_emotion = 0
    image_size_u_v = 28
...
def crop_optical_flow_block():
    ...
    # print(face_block_coordinates_dict)
    # Get train dataset
    whole_optical_flow_path = r'E:\HTnet\HTNet-master\datasets\STSNet_whole_norm_u_v_os'
    whole_optical_flow_imgs = os.listdir(whole_optical_flow_path)
    four_parts_optical_flow_imgs = {} #dictionary to store the cropped regions for each optical flow image.
    ...
    # print((four_parts_optical_flow_imgs['spNO.189_f_150.png'][0]))
...
def main(config):
    learning_rate = 0.00005 #Defines the step size for updating model weights during training.
    batch_size = 256 #Number of samples processed simultaneously.
    epochs = 800
    all_accuracy_dict = {}
    ...
    total_gt = [] #ground truth or actual truth
    total_pred = [] 
    best_total_pred = []

    t = time.time()

    main_path = r'E:\HTnet\HTNet-master\datasets\three_norm_u_v_os'
    ...
    for n_subName in subName:
        ...
        y_train = [] # Label lists
        y_test = [] 
        four_parts_train = [] # Feature lists
        four_parts_test = []
        # Get train dataset
        ...
        # Get test dataset
        expression = os.listdir(main_path + '/' + n_subName + '/u_test') #Collects the list of expression categories (labels).
        for n_expression in expression:
            img = os.listdir(main_path + '/' + n_subName + '/u_test/' + n_expression) #Iterates through each expression category and Retrieves all image filenames
        ...
        #[num_samples, height, width, channels] -> [batch_size, channels, height, width] (batch_size and num_samples are same)
        dataset_train = TensorDataset(four_parts_train, y_train) #pairing features and labels
        train_dl = DataLoader(dataset_train, batch_size=batch_size)#iterate over dataset in mini-batches
        ...
        for epoch in range(1, epochs + 1):
            if (config.train):
                ...
                for batch in train_dl:
                    ...
                    x = batch[0].to(device) #images
                    y = batch[1].to(device) #true lables
                    yhat = model(x) #rawmodel Output before softmax
                    loss = loss_fn(yhat, y)
                    loss.backward()
                    optimizer.step()
            ...
            #### best result This snippet stores the best class predictions for each subject across different epochs, 
            # ensuring that the best-performing model's predictions are retained
